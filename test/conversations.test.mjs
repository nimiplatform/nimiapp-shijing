...
test('SJG-ASTRO-05 (extended): CONVERSATION_SYSTEM_PROMPT bans 吉凶 and luck score', () => {
  assert.ok(CONVERSATION_SYSTEM_PROMPT.includes('永不预言吉凶'));
  assert.ok(CONVERSATION_SYSTEM_PROMPT.includes('永不输出 luck score'));
});

test('SJG-DATA-08: conversation chat bridge returns ok=true on a normal text generator', async () => {
  ...
  const result = await bridge.send({ user_message: 'hello', model_id: 'auto' });
...
test('SJG-DATA-08: conversation chat bridge surfaces generator_call_failed when generator throws', async () => {
  ...
  const result = await bridge.send({ user_message: 'x', model_id: 'auto' });
