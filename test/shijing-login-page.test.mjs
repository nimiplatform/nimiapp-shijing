...
test('ShiJing renderer contains no login, logout, OAuth, or account-control surface', () => {
  const authProviderSource = readFileSync(
    new URL('../src/shell/app-shell/auth-provider.tsx', import.meta.url),
    'utf8',
  );
  ...
  assert.doesNotMatch(authProviderSource, /login|logout|OAuth|beginLogin|completeLogin/i);
  assert.match(authProviderSource, /protectedSession/);
});
