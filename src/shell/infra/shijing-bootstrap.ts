...
export const SHIJING_RUNTIME_APP_ID = 'app.nimi.shijing';
export const SHIJING_RUNTIME_APP_INSTANCE_ID = `${SHIJING_RUNTIME_APP_ID}.local-first-party`;
export const SHIJING_RUNTIME_DEVICE_ID = 'local-first-party-device';
...
async function doRunShijingBootstrap(): Promise<void> {
  ...
  try {
    ...
    clearPlatformClient();
    const platformClient = await buildShijingPlatformClient(runtimeDefaults.realm.realmBaseUrl).catch(
      (error) => {
        logRendererEvent({
          level: 'warn',
          area: 'shijing-bootstrap.runtime-client',
