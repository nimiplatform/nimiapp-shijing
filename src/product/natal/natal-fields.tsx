  ...
  return (
    <>
      <div className="sjp-field">
        <label className="sjp-label" htmlFor={id('calendar')}>{copy.natal.calendar}</label>
        <SjpSelect
          id={id('calendar')}
          value={draft.calendar_system}
          onValueChange={(v) => onChange('calendar_system', v as SelfNatalDraft['calendar_system'])}
          options={CALENDAR_SYSTEMS.map((cs) => ({ value: cs, label: copy.calendarSystemLabels[cs] }))}
        />
      </div>

      <div className="sjp-field">
        <label className="sjp-label" htmlFor={id('sex')}>{copy.natal.sex}</label>
        <SjpSelect
          id={id('sex')}
          value={draft.calculation_sex}
          onValueChange={(v) => onChange('calculation_sex', v as SelfNatalDraft['calculation_sex'])}
          options={CALCULATION_SEXES.map((s) => ({ value: s, label: copy.calculationSexLabels[s] }))}
        />
      </div>

      <div className="sjp-field">
        <label className="sjp-label" htmlFor={id('date')}>{copy.natal.birthDate}</label>
        {draft.calendar_system === 'gregorian' ? (
      ...
      <div className="sjp-field">
        <label className="sjp-label" htmlFor={id('place')}>{copy.natal.birthPlace}</label>
        <PlaceAutocomplete
          id={id('place')}
          placeholder={copy.natal.birthPlacePlaceholder}
      ...
      <div className="sjp-field">
        <label className="sjp-label" htmlFor={id('time')}>{copy.natal.birthTime}</label>
        <div
      ...
      <div className="sjp-field">
        ...
        <label className="sjp-inline-check">
          ...
          />
          {copy.natal.unknownTime}
        </label>
      </div>

      {dstActive ? (
        <p className="sjp-note sjp-note--warn">{copy.natal.dstWarning}</p>
      ) : null}
          ...
          {calibrationOpen ? (
            <div className="sjp-collapse__body" id={id('calibration')}>
              <p className="sjp-note">
                {copy.natal.calibrationDescription}
              </p>
              <div className="sjp-grid sjp-grid--triple">
                <div className="sjp-field">
                  <label className="sjp-label" htmlFor={id('lat')}>{copy.natal.latitude}</label>
                  <input
                    id={id('lat')}
                    type="text"
                    className="sjp-input sjp-input--mono"
                    placeholder={copy.natal.latitudePlaceholder}
                    value={draft.latitude}
                    onChange={(e) => onChange('latitude', e.currentTarget.value)}
                  />
                </div>
                <div className="sjp-field">
                  <label className="sjp-label" htmlFor={id('lng')}>{copy.natal.longitude}</label>
                  <input
                    id={id('lng')}
                    type="text"
                    className="sjp-input sjp-input--mono"
                    placeholder={copy.natal.longitudePlaceholder}
                    value={draft.longitude}
                    onChange={(e) => onChange('longitude', e.currentTarget.value)}
                  />
                </div>
                <div className="sjp-field">
                  <label className="sjp-label" htmlFor={id('tz')}>{copy.natal.timeZone}</label>
                  <input
                    id={id('tz')}
                    type="text"
                    className="sjp-input sjp-input--mono"
                    placeholder={copy.natal.timeZonePlaceholder}
