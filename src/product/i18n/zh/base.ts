...
export const ZH_BASE_COPY: Omit<ProductCopy, 'rijing' | 'yuejing' | 'hejing' | 'shijing' | 'mingjing'> = {
brandName: '时镜',
brandSub: 'ShiJing',
mirrorKindLabels: {
  rijing: '日镜',
...
shell: {
  ...
  snapshotInvalid: (code) =>
    `数据快照校验未通过: ${code}。请点击右上角头像打开"设置 → 本地数据与诊断"清理已存数据后再试。`,
...
methodProfile: {
  title: '推演方法',
  algorithm: '命理算法',
  switched: (methodLabel) => `已切换为${methodLabel}`,
  capabilities: {
    title: '功能支持声明',
    current: '当前',
...
aiConfig: {
  title: 'AI 模型配置',
  description: '配置时镜 App 的 text.generate 意向，供 Runtime AI 文案和咨询使用',
  runtimeNotReady: 'Runtime 未就绪',
  runtimeUnavailable: 'Runtime 不可用',
  runtimeBootstrapPending: 'Runtime bootstrap 尚未完成。',
  needsTarget: '需要配置',
  missingTarget: '缺少文本生成配置',
  missingTargetDetail: '时镜 App AIConfig 缺少可执行的 text.generate 意向时，生成会 fail-close。',
  configured: '已配置',
...
citationDrawer: {
  ...
  method: '推演方法',
  referenceRange: '参考范围',
  consultationBasis: '问镜依据',
  capturedAt: '依据冻结',
  localIntegrity: '本地校验',
  localIntegrityValue: '已冻结出生资料、关注标签、引用资料与回答偏好；历史解读不会随资料变更自动改写。',
  scopeDate: (date, timeZone) => `${date}（${timeZone}）`,
  scopeRange: (startDate, endDate, timeZone) => `${startDate} 至 ${endDate}（${timeZone}）`,
  natalAnchor: (anchorYear, timeZone) => `${anchorYear} 年本命参照（${timeZone}）`,
  relationshipAnchor: (anchorYear, timeZone) => `${anchorYear} 年合盘参照（${timeZone}）`,
  consultationSourceCount: (count) => `引用 ${count} 份已生成解读`,
  questionWindow: (startDate, endDate, timeZone) => `问题时间 ${startDate} 至 ${endDate}（${timeZone}）`,
  citedMemories: '引用记忆',
  citedPlans: '引用计划',
},
importToShijing: {
